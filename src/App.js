...
import { fetchUsers, addUser } from "./requests";
import {
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    fetchUsers().then(users => {
      ...
      this.setState({ users, userChatData: users }, () => {
        if (sessionStorage.getItem('loggedInUser')) {
          const user = sessionStorage.getItem('loggedInUser');
          if (user) {
            const _user = JSON.parse(user);
            this.onUserClicked({ user: _user });
          }
  ...
  setupSocketListeners() {
    ...
    this.socket.on("updateUsers", this.updateUsers.bind(this));
  }
  ...
  onMessageRecieved(message) {
    ...
    this.setState({ userChatData }, () => {
      ...
      localStorage.setItem('userMsgs-' + usr.id, JSON.stringify(this.state.userChatData));
    });
  }
  ...
  onUserClicked(e) {
    ...
    this.setState({ user, signInModalShow: false, userChatData, showaddUser: false }, () => {
      const user = this.state.user;
      const usermsgs = localStorage.getItem('userMsgs-' + user.id);
      if (usermsgs) {
        this.setState({ userChatData: JSON.parse(usermsgs) });
      }
    });
  }
  ...
  render() {
    ...
    return (
      <div>
        <NavBar signedInUser={this.state.user} logout={() => this.logout()} />
        <Grid>
        ...
        <AddUser users={this.state.users}
          onUserClicked={this.onUserClicked.bind(this)}
          addUser={this.addUserFn} openModal={this.state.showaddUser} onClose={() => this.setState({ showaddUser: false })} />
        {/* <ErrorModal
  ...
  addUserFn = (username) => {
    addUser(username).then(res => {
      fetchUsers().then(users => {
        ...
        sessionStorage.setItem('loggedInUser', JSON.stringify(user));
        let userChatData = this.state.users.filter(u => u.id !== user.id);
        const index = this.state.users.findIndex(x => x.id === res.id);
        this.setState({ users, userChatData: users, user, signInModalShow: false, userChatData, selectedUserIndex: index });
